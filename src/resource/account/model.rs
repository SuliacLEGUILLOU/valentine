...
use crate::engine::password_engine::{check_password, hash_password};
...
impl Model {
    ...
    pub fn get_by_id(conn: Pool, id: &str) -> Vec<Model> {
        ...
        for row in &conn
            .query(
                "SELECT id, name, email, password FROM account WHERE id=$1",
                &[&id],
            )
            .unwrap()
        {
    ...
    pub fn get_all(conn: Pool) -> Vec<Model> {
        ...
        for row in &conn
            .query(
                "SELECT id, name, email, password FROM account LIMIT 100",
                &[],
            )
            .unwrap()
        {
    ...
    pub fn insert(&mut self, conn: Pool) {
        ...
        conn.execute(
            "INSERT INTO account (id, name, email, password) VALUES ($1, $2, $3, $4)",
    ...
    pub fn delete(&self, conn: Pool) {
        conn.execute("DELETE FROM account WHERE id=$1", &[&self.id])
            .unwrap();
    }
}
